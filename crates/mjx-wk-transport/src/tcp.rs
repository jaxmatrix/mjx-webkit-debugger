...
//!
//! On non-Unix platforms (Windows for v0.1), this backend is stubbed: WebKitGTK
//! inspector TCP requires GLib and arrives with CDP attach in a later phase.

use std::path::{Path, PathBuf};

use crate::discovery::default_webkit_library;
...
mod unix {
    ...
    use super::TcpInspectorServer;
    use crate::discovery::{
        Discovery, SocketEvent, TargetDescriptor, TargetKey, backend_commands_hash,
        decode_messages, descriptors_from_target_list, encode_message,
    };
...
#[cfg(unix)]
pub use unix::TcpTransport;

#[cfg(not(unix))]
use async_trait::async_trait;

#[cfg(not(unix))]
use crate::discovery::{Discovery, TargetDescriptor, TargetKey, WEBKITGTK_TCP_UNAVAILABLE};
#[cfg(not(unix))]
use crate::TransportError;

#[cfg(not(unix))]
fn platform_unavailable(endpoint: &str) -> TransportError {
    TransportError::Discovery {
...
#[cfg(not(unix))]
/// Placeholder transport type; [`TcpInspectorServer::attach`] always fails on Windows.
#[derive(Debug)]
pub struct TcpTransport;
...
impl TcpInspectorServer {
    ...
    pub async fn attach(&self, _key: &TargetKey) -> Result<TcpTransport, TransportError> {
        Err(platform_unavailable(&self.address))
    }
}
